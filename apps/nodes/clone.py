def clone_tables(node, clone, using):
    ...
    if node.kind == node.Kind.OUTPUT and hasattr(node, "table"):
        node.table.make_clone({"workflow_node": clone}, sub_clone=True, using=using)
    elif node.intermediate_table:
        node.intermediate_table.make_clone(
            {"intermediate_node": clone}, sub_clone=True, using=using
        )
    elif node.cache_table:
        node.cache_table.make_clone({"cache_node": clone}, sub_clone=True, using=using)
