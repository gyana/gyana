...
def dateslicer_of_team(user, pk, *args, **kwargs):
    date_slicer = get_object_or_404(DateSlicer, pk=pk)
    project = date_slicer.dashboard.project
    return user_can_access_project(user, project)


login_and_dateslicer_required = login_and_permission_to_access(dateslicer_of_team)


def dateslicer_of_public(view_func):
    """Returns a decorator that checks whether a dateslicer belongs
    to a public or password protected dashboard."""

    @wraps(view_func)
    def decorator(request, *args, **kwargs):

        date_slicer = get_object_or_404(DateSlicer, pk=kwargs["pk"])
        dashboard = date_slicer.dashboard
