...
def test_duplicate_simple_project(
    ...
    assert (
        f"CREATE OR REPLACE TABLE {duplicate_table.table_id} as (SELECT * FROM {table.table_id})"
        in call_queries
    )
    assert (
        f"CREATE OR REPLACE TABLE {duplicate_output_node.table.table_id} as (SELECT * FROM {output_node.table.table_id})"
        in call_queries
    )
