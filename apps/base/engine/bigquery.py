...
def _load_table(upload: "Upload", table: "Table", client, **job_kwargs):
    ...
    load_job = client.load_table_from_uri(
        upload.gcs_uri, table.table_id, job_config=job_config
    )
...
class BigQueryClient(BaseClient):
    ...
    def _get_bigquery_object(self, table_id):
        return bigquery().get_table(table_id)
    ...
    def import_table_from_upload(self, table: "Table", upload: "Upload"):
        ...
        if bq_table_schema_is_string_only(self._get_bigquery_object(table.table_id)):
            ...
            header_query = client.query(
                f"select * from (select * from {temp_table_id} except distinct select * from {table.table_id}) limit 1",
    ...
    def import_table_from_customapi(self, table: "Table", customapi: "CustomApi"):
        ...
        load_job = client.load_table_from_uri(
            customapi.gcs_uri, table.table_id, job_config=job_config
        )
    ...
    def get_table_size(self, table: "Table"):
        return self._get_bigquery_object(table.table_id).num_rows

    def get_source_metadata(self, table: "Table"):
        bq_obj = self._get_bigquery_object(table.table_id)
    ...
    def import_table_from_sheet(self, table: "Table", sheet: "Sheet"):
        ...
        if bq_table_schema_is_string_only(self._get_bigquery_object(table.table_id)):
            ...
            header_query = client.query(
                f"select * from (select * from {table.table_id} except distinct select * from {temp_table_id}) limit 1",
                job_config=job_config,
            )
